<h3 class="text-success text-center">All Orders</h3>
<table class="table table-bordered mt-5">
    <thead>

    <?php 
    $get_orders="Select * from `user_orders`";
    $result=mysqli_query($con,$get_orders);
    $row_count=mysqli_num_rows($result);
    echo "<tr class='bg-secondary text-light text-center'>
    <th>Sr.no.</th>
    <th>Due Amount</th>
    <th>Invoice Number</th>
    <th>Quantity</th>
    <th>Item & Customization Details</th>
    <th>Order Date</th>
    <th>Status</th>
    <th>Delete</th>
</tr>
</thead>
<tbody class='text-center'>";

//if the above data fetched is 0 then
if($row_count==0){
    echo "<h2 class='text-danger text-center mt-5'>No orders yet!</h2>";
}else{
    $number=0;
    while($row_data=mysqli_fetch_array($result)){
        $order_id=$row_data['order_id'];
        $user_id=$row_data['user_id'];
        $amount_due=$row_data['amount_due'];
        $invoice_number=$row_data['invoice_number'];
        $quantity=$row_data['quantity'];
        $customization_description=$row_data['customization_description'];
        $order_date=$row_data['order_date'];
        $order_status=$row_data['order_status'];
        $number++;
        ?>
        <tr>
        <td><?php echo $number;?></td>
        <td><?php echo $amount_due;?></td>
        <td><?php echo $invoice_number;?></td>
        <td><?php echo $quantity;?></td>
        <td><?php echo $customization_description;?></td>
        <td><?php echo $order_date;?></td>
        <td><?php echo $order_status;?></td>
        
        <td><a href="admin_panel.php?delete_order=<?php echo $order_id?>" type="button" class="text-dark" data-toggle="modal" data-target="#exampleModalLong"><i class="fa-solid fa-trash"></i></a></td> 
        
    </tr>
 <?php   
    }
}
?>
   
        
    </tbody>
</table>
<!-- Modal/popup confirmation -->
<div class="modal fade" id="exampleModalLong" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      
      <div class="modal-body">
        <h4>Are you sure you want to delete this?</h4>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-light border border-primary" data-dismiss="modal"><a href="./admin_panel.php?list_orders" class="text-decoration-none">No</a></button>
        <button type="button" class="btn btn-primary"><a href='admin_panel.php?delete_order=<?php echo $order_id?>' class="text-light text-decoration-none">Yes</a></button>
      </div>
    </div>
  </div>
</div>
<?php
// Fetch data to determine products sold
$get_demand_query = "SELECT p.product_id, p.product_title, uo.order_id, SUM(uo.quantity) AS total_quantity_sold 
                     FROM user_orders uo
                     INNER JOIN products p ON uo.product_id = p.product_id
                     GROUP BY uo.product_id, uo.order_id
                     ORDER BY total_quantity_sold DESC";
$demand_result = mysqli_query($con, $get_demand_query);
?>

<h3 class="text-success text-center">Product Sales Report</h3>
<table class="table table-bordered mt-5">
    <thead>
        <tr class='bg-secondary text-light text-center'>
            <th>Product ID</th>
            <th>Product Name</th>
            <th>Order ID</th>
            <th>Total Quantity Sold</th>
        </tr>
    </thead>
    <tbody class='text-center'>
        <?php
        while ($row = mysqli_fetch_assoc($demand_result)) {
            echo "<tr>";
            echo "<td>" . $row['product_id'] . "</td>";
            echo "<td>" . $row['product_title'] . "</td>";
            echo "<td>" . $row['order_id'] . "</td>";
            echo "<td>" . $row['total_quantity_sold'] . "</td>";
            echo "</tr>";
        }
        ?>
    </tbody>
</table>
